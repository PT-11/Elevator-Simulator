 ...
 * you are running and the instance of your messages class.
 * Create a thread with this class and call the start() method to start it. 
 * There are five optional methods that can be used to add more information 
 * in the display. Their use is related to how much of the FrameGUI you have
 * implemented in the Elevator class. The methods are; 
 * drawValueAbove - draws a sting above the elevator. 
 * drawValueToSide - draws a string value to the right of the elevator.
 * drawStringToSide - draws a string value to the right of the elevator,
 * below the previous one.
 * drawStringInBase - draws a string in the base below each elevator.
 * getStateColor - Sets the state colour for the elevator. doMouseHighlight -
 * draws the elevator number when the mouse is held above Comment out the ones
 * you want to use.
 *
 */
public class FrameView implements Runnable
{
	...
	String titleName;								// Name for Title

	private final Timer TIMER; 						// Timer to draw the graphics
	private Dimension liftDimension; 				// The lift dimensions

	private Dimension levelLabelDimension; 			// Label dimensions
	private FontMetrics fm; 						// Font metrics for font used in graphics
	private Messages messages;						// Contains messaging services
	private int[] floors;							// Current floor location
	private int[] state;							//Current elevator states
	private int[] people;							// People in each elevator
	
	private ArrayList<String> command;				// Commands to display
	 ...
	 * @param messages	Messaging class instance
	 */

	public FrameView(String titleName, final int minFloor, final int maxFloor, final int numLift, Messages messages)
	{
		...
		TIMER = new Timer(250, null); // Timer for repaints
		
		// Initialise arrays for display values
		floors = new int[numLift];
		
		for(int value : floors)
			value = 1;
		...
		state = new int[numLift];
		
		for(int value : state)
			value = 2;
		
		people = new int[numLift];			// default to 0
		
		info = new String[numLift];
	}
	...
	private void startGraphics()
	{
		...
		jframe.add(new JComponent()
		{
			...
			public void paintComponent(Graphics graphics)
			{
				...
				{
					// These are example input messages for the graphics. You need to do one for the floors
					// here it uses 'F' for the floor. Since the elevators start at 1 I am adding 1 to i.
					// The rest are optional if you want to display the values.
					if ((message = messages.getMessage(QueueType.OUTPUT, i + 1, 'F', true)) != null) 
						floors[i] = Integer.parseInt(message);	
					
					if ((message = messages.getMessage(QueueType.OUTPUT, i + 1, 'C', true)) != null) 
						if (message.length() > 0)
							command.set(i, message);	
					
					if ((message = messages.getMessage(QueueType.OUTPUT, i + 1, 'P', true)) != null) {  
						lastPeople = people[i];
						people[i] = Integer.parseInt(message);
							
					}
			
					if ((message = messages.getMessage(QueueType.OUTPUT, i + 1, 'I', true)) != null)  
						info[i]= message;
					...
					liftPoint.y = (getHeight() - liftDimension.height - gHeight)
					        - Math.abs((floors[i] + adjustment) * (levelHeight));

					// Optional display text values for lift
					// Uncomment any display option you want
					drawValueToSide("" + floors[i], graphics); // current floor
					drawValueAbove(graphics, i, liftPoint, "" + people[i]);
					drawStringToSide(info[i], graphics);
					drawStringInBase(this, graphics, command.get(i), liftPoint);
					...
					doMouseHighlight(graphics, getHeight(), floors[i]);
	...
	private String getPeopleMovement(int last, int next) {
		String message = "";
	...
	private Color getStateColor(int i)
	{
		...
		if ((message = messages.getMessage(QueueType.OUTPUT, i + 1, 'S', true)) != null) { 
			try
			{
				state[i] = Integer.parseInt(message);	
			} catch(NumberFormatException ne) {
				System.out.println("Excpetion " + message);
		...
		case 2:
			...
			break;
		default:
			throw new IllegalArgumentException("Unexpected value: " + state[i]);
		} 
	 ...
	 * @param liftPoint
	 */
	private void drawValueAbove(Graphics graphics, int liftNo, Point liftPoint, String text)
	{
		// Get passenger number and display it above elevator
		Dimension liftLabelTextDimension = new Dimension(
		        (int) fm.getStringBounds(text, graphics).getWidth(),
		        (int) fm.getStringBounds(text, graphics).getHeight());
	 ...
	 * @param height
	 */
	private void doMouseHighlight(Graphics graphics, int height, int currentFloor)
	{
		...
		for (int i = 0; i < numLift; i++)
		{
			...
			mLiftPoint.y = (height - liftDimension.height - gHeight)
			        - (currentFloor + adjustment) * levelHeight;
